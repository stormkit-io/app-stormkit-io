...
const ExitStatus = ({ code, iconOnly, className }) =>
  code === 0 ? (
    <span className={cn("text-green-50", className)}>
      <span className="fas fa-check-circle" /> {!iconOnly && "Success"}
    </span>
  ) : code !== null ? (
    <span className={cn("text-red-50", className)}>
      <span className="fas fa-times-circle" /> {!iconOnly && "Failed"}
    </span>
  ) : (
    <span className={cn("text-blue-40", className)}>
